...
def build_model(env, args, device):
    ...
    if "ToM2C" in name and "RA" in args.env:
...
class ToM2C_multi(torch.nn.Module):
    ...
    def __init__(self, obs_space, action_spaces, args, device=torch.device('cpu')):
        ...
        self.ToM_GRU = RNN(_state_size, lstm_out, 1, device, 'GRU')
...
class ToM2C_single(torch.nn.Module):
    ...
    def forward(self, multi_obs, self_hiddens, ToM_hiddens, test=False, available_actions = None, train_comm=False):
        ...
        self_pos = Variable(multi_obs[:, :, :self._state_size], requires_grad=True)
        multi_obs = Variable(multi_obs[:, :, self._state_size:].reshape(batch_size, num_agents, num_targets, self._state_size), requires_grad=True)
        ...
        camera_states = cam_states_relative # torch.cat((cam_states_relative, self_vel_duplicate), -1)
