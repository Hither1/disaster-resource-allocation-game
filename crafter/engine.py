...
class World:
  ...
  def update_OO(self):
    for resource in ['food', 'drink']:
        self.shelter.OO[resource] = sum(self.warehouse.AO[resource]) + sum(self.shelter.AS[resource])
        self.station.OO[resource] = sum(self.warehouse.AO[resource]) + sum(self.shelter.AS[resource])

        self.warehouse.OO[resource] = sum(self.warehouse.AS[resource])
        
    for resource in ['staff']:
        self.shelter.OO[resource] = sum(self.warehouse.AO[resource]) + sum(self.shelter.AS[resource])
        self.warehouse.OO[resource] = sum(self.warehouse.AO[resource]) + sum(self.shelter.AS[resource])
        # Upstream
...
class ItemView:
  ...
  def __call__(self, inventory, unit):
    ...
    for index, (item, amount) in enumerate(inventory.items()):
      if amount < 1:
        continue
      self._item(canvas, index + 2, item, unit)
      self._amount(canvas, index + 2, amount, unit)
    return canvas
  ...
  def _amount(self, canvas, index, amount, unit):
    ...
    if amount in list(range(10)):
      texture = self._textures.get(str(amount), 0.6 * unit)
      _draw_alpha(canvas, pos, texture)
    elif amount in list(range(100)):
      text = str(amount)
      texture = self._textures.get(text[0], 0.3 * unit)
      pos[1] += 15
      _draw_alpha(canvas, pos, texture)

      texture = self._textures.get(text[1], 0.3 * unit)
      pos[0] += 20
      _draw_alpha(canvas, pos, texture)
    else:
      texture = self._textures.get('unknown', 0.6 * unit)
      _draw_alpha(canvas, pos, texture)

class ChatView:
  def __init__(self, grid):
    self._grid = np.array(grid)
    self.info = ["Warehouse->Station: Hey, we are in urgent need of staff, we want you to send 3 staff here as soon as possible.",
                "Station->Warehouse: Hi, can you send 10 supplies in 5 days? Thanks.", 
                "Shelter->Station: Hello, we may need 5 helpers in a few days, all the best.",
                ]
  ...
  def __call__(self, unit):
    unit = np.array(unit)
    canvas = np.zeros(tuple((self._grid * unit)[::-1]) + (3,), np.uint8)
    canvas += 255

    cv2.putText(canvas, "Chat History", (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2)
    if len(self.info) == 0:
        return canvas.transpose((1, 0, 2))
    
    # game_step = self.game_step if game_step is None else game_step
    result = "Most recent {} messages of the player's in game:\n\n".format(len(self.info))
    for i, info in enumerate(self.info[-15:]):
      entity, msg = info.split(":")
      cv2.putText(canvas, entity, (15, (i + 1) * 50), cv2.FONT_HERSHEY_SIMPLEX, .65, (0, 0, 0), 2)
      cv2.putText(canvas, msg, (15, (i + 1) * 50 + 25), cv2.FONT_HERSHEY_SIMPLEX, .65, (0, 0, 0), 2)
