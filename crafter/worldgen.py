...
def _set_object(world, pos, players, tunnels):
  x, y = pos
  uniform = world.random.uniform
  dist = [(np.sqrt((x - player.pos[0]) ** 2 + (y - player.pos[1]) ** 2)) for player in players]
  material, _ = world[x, y]
  if material not in constants.walkable:
    pass
  # elif all(x > 3 for x in dist)  and material == 'grass' and uniform() > 0.985:
  #   world.add(objects.Cow(world, (x, y)))
  # elif all(x > 10 for x in dist) and uniform() > 0.993:
  #   world.add(objects.Zombie(world, (x, y), players[0]))
  # elif material == 'path' and tunnels[x, y] and uniform() > 0.95:
  #   world.add(objects.Skeleton(world, (x, y), players[0]))
