...
class Viewer(object):
    ...
    def render(self, return_rgb_array=False):
        ...
        if return_rgb_array:
            ...
            # In https://github.com/openai/gym-http-api/issues/2, we discovered that someone using Xmonad on Arch was having a window of size 598 x 398, though a 600 x 400 window
            # was requested. (Guess Xmonad was preserving a pixel for the boundary.) So we use the buffer height/width rather than the requested one.
            arr = arr.reshape(buffer.height, buffer.width, 4)
            arr = arr[::-1,:,0:3]
        self.window.flip()
        self.onetime_geoms = []
